...
public interface DestinationRepository extends JpaRepository<Destination, Long> {
    ...
    @Query("select d from Destination d inner join DestinationTypeDTO dt on d.type = dt where dt.name = (?1)")
    ...
    @Query("select ds from DestinationStatusDTO ds join Destination d where d.id = ?1")
    public Optional<DestinationStatus> getStatus(Long DID);

    @Query("select d from Destination d join DestinationTypeDTO dt where dt = ?1")
